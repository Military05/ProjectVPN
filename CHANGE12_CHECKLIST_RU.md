...
- Отдельный PostgreSQL 16.14 run [`35252712448`](https://github.com/Military05/ProjectVPN/actions/runs/35252712448):
  `6 passed`, без skipped.
- Финальный real-service run [`35254302019`](https://github.com/Military05/ProjectVPN/actions/runs/35254302019):
  полный набор `337 passed`, `0 skipped`; clean Docker stack и admin UI/API smoke
  также прошли.
- CHANGE-12 полностью подтверждён; команды ниже оставлены для добровольного
  повторения проверки на ноутбуке.
...
- [x] Unit-тесты CHANGE-12 прошли.
- [x] PostgreSQL integration-тесты действительно запустились, а не были пропущены.
- [x] Для одновременного создания тарифа результат — один `201` и один `409`.
- [x] Для одновременного создания сервера результат — один `201` и один `409`.
- [x] Для одновременного создания узла результат — один `201` и один `409`.
- [x] В базе ровно один узел и ровно один credential этого узла.
- [x] Для двух одинаковых endpoint результат — один `201` и один `409`, включая
      вариант с пустыми необязательными полями.
- [x] Ошибка при создании спецификации тарифа откатывает сам тариф.
- [x] Ошибка при создании credential откатывает сам узел.
- [x] Нарушения `CHECK` и `FOREIGN KEY` не возвращаются как ложный `409`.
- [x] `compileall` прошёл.
- [x] Полный pytest прошёл без падений и skipped.
- [x] Health и административная панель не падают в middleware с `_IncludedRouter`.
- [x] `.env`, реальные токены и пароли не попали в Git.

CHANGE-12 полностью подтверждён на настоящем PostgreSQL 16.14. Локальное повторение
чек-листа не является блокером; оно нужно только для проверки конкретного ноутбука
или локальной Docker-конфигурации.
